...
<body>
    ...
    function loadPage(pagina = 1) {
        ...
        // Mostrar loader
        document.getElementById('manga-results').innerHTML = '<div class="loading">Cargando mangas...</div>';
        ...
        fetch(`ajax_filtrar_mangas.php?${params.toString()}`)
            .then(response => response.text())
            .then(html => {
                document.getElementById('manga-results').innerHTML = html;
                // Desplazarse suavemente al inicio de los resultados
                window.scrollTo({
                    top: document.getElementById('manga-results').offsetTop - 20,
                    behavior: 'smooth'
                });
            })
            .catch(error => {
                console.error('Error:', error);
                document.getElementById('manga-results').innerHTML = '<div class="error">Error al cargar los mangas</div>';
            });
    }
