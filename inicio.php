...
<head>
    ...
    <style>
        ...
        .Novela-Card:hover .Novela-Image img {
            transform: scale(1.05);
        }
        
        /* ESTILOS CORREGIDOS PARA LOS BADGES */
        .Status-Badge {
            position: absolute;
            top: 10px;
            right: 10px;
        ...
        /* Usando atributos data-status para mejor precisión */
        .Status-Badge[data-status="En emisión"] { background: #4CAF50; }
        .Status-Badge[data-status="Finalizado"] { background:rgb(255, 17, 0); }
...
<body>
    ...
    <div class="Body">
        <div class="Container">
            ...
            <div id="novela-results">
                ...
                <div class="loading">Cargando novelas...</div>
            </div>
        </div>
    </div>
    ...
    <!-- JavaScript -->
    <script>
    // Función para cargar una página específica con los filtros actuales
    function loadPage(pagina = 1) {
        ...
        // Mostrar loader
        document.getElementById('novela-results').innerHTML = '<div class="loading">Cargando novelas...</div>';
        ...
        fetch(`ajax_filtrar_novelas.php?${params.toString()}`)
            .then(response => response.text())
            .then(html => {
                document.getElementById('novela-results').innerHTML = html;
                // Desplazarse suavemente al inicio de los resultados
                window.scrollTo({
                    top: document.getElementById('novela-results').offsetTop - 20,
                    behavior: 'smooth'
                });
            })
            .catch(error => {
                console.error('Error:', error);
                document.getElementById('novela-results').innerHTML = '<div class="error">Error al cargar los novelas</div>';
            });
    }
