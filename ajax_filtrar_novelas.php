...
if ($result->num_rows > 0) {
    echo '<div class="List-Novelas">';
    while($novela = $result->fetch_assoc()) {
        echo '
        <div class="Novela-Card">
            <a href="novela_detalle.php?id='.$novela['id'].'">
                <div class="Novela-Image">
                    <img src="'.htmlspecialchars($novela['imagen']).'" alt="'.htmlspecialchars($novela['nombre']).'" loading="lazy">
                    <span class="Status-Badge" data-status="'.htmlspecialchars($novela['estado']).'">'.htmlspecialchars($novela['estado']).'</span>
                </div>
                <div class="Novela-Info">
                    <h3 class="Novela-Title">'.htmlspecialchars($novela['nombre']).'</h3>
